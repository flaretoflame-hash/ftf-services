...
const ALLOWED_ORIGIN = 'https://flaretoflame-hash.github.io';  // W3: locked to site origin
...
function makeOtp() {
  // W2: cryptographically secure, unbiased digits (reject bytes >= 250)
  let s = '';
  while (s.length < OTP_LENGTH) {
    const b = new Uint8Array(1);
    crypto.getRandomValues(b);
    if (b[0] < 250) s += (b[0] % 10).toString();
  }
  return s;
}
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (request.method === 'POST' && url.pathname === '/send-otp') {
        ...
        // W4: cap sends (max 3 per phone / 10 min) to stop OTP + WhatsApp spam
        const sentKey = 'otpsent:' + phone;
        const sentCount = parseInt((await env.OTP_KV.get(sentKey)) || '0', 10);
        if (sentCount >= 3) {
          return json({ ok: false, error: 'Too many code requests. Please wait a few minutes.' }, 429);
        }
        await env.OTP_KV.put(sentKey, String(sentCount + 1), { expirationTtl: 600 });
      ...
      if (request.method === 'POST' && url.pathname === '/verify-otp') {
        ...
        // W1: brute-force lockout — max 5 wrong tries per phone, then 15-min lock
        const failKey = 'otpfail:' + phone;
        const fails = parseInt((await env.OTP_KV.get(failKey)) || '0', 10);
        if (fails >= 5) {
        ...
        if (stored !== otp) {
          await env.OTP_KV.put(failKey, String(fails + 1), { expirationTtl: 900 });
          return json({ ok: false, error: 'Incorrect code' }, 400);
        }

        // Correct — clear fail counter + delete OTP so it can't be reused
        await env.OTP_KV.delete(failKey);
